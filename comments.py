...
if app_mode == "Toxicity comments detection":
    ...
    metric_col1.metric(label="Model Architecture", value="Fine-Tuned DistilBERT", delta="Focal Loss Optimized")
    metric_col2.metric(label="Balanced Accuracy", value="93.84%", delta="Robust to Imbalance")
    metric_col3.metric(label="Discriminative Area (ROC-AUC)", value="94.21%", delta="High Predictive Power")
    metric_col4.metric(label="Mean Processing Latency", value="18ms", delta=device_info)
    st.markdown("---")
    ...
    with plot_col1:
        ...
        train_loss = np.array([0.3821, 0.2104, 0.1412, 0.0983, 0.0651])
        val_loss = np.array([0.2943, 0.1852, 0.1301, 0.1044, 0.0892])
        ...
        ax.set_ylabel("Focal Loss Scores")
    ...
    with plot_col2:
        ...
        st.subheader("🧪 Sample Example Test Cases")
    mock_cases = pd.DataFrame({
        ...
        "Example Model Response": ["Pass (Approved)", "Fail (Flagged)", "Fail (Flagged)", "Pass (Approved)"]
    })
    st.table(mock_cases)